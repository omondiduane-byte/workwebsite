...
export const db = {
  from: (table: string) => ({
    ...
    insert: <T = unknown>(payload: T | T[]) => handle<T>(supabase.from(table).insert(payload as never)),
    update: <T = unknown>(payload: Partial<T>) => ({ eq: (col: string, val: unknown) => handle<T>(supabase.from(table).update(payload as never).eq(col, val)) }),
